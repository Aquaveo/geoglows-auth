...
- `src/bootstrap/index.ts` — `bootstrapAuth`, the one-call vanilla entry point (`@geoglows/geoglows-auth/bootstrap`). Owns the Supabase client, the sign-in modal, the navbar slot, recovery-URL handling and the `onAuthStateChange` lifecycle. Also owns the **connect budget** (1.8.0): bounded attempts/timeout/wall-clock against the account service, classified retries, jittered backoff, an error state that recovers on evidence (`online`, tab focus, an auth event, the retry button), and explicit control of Supabase's token-refresh ticker via `startAutoRefresh()` / `stopAutoRefresh()`. See `docs/solutions/best-practices/bounded-auth-connect-budget-2026-09-01.md`
- `src/core/retry.ts` — `isTransientError` (transient vs permanent failure classification: 5xx/408/429/transport failures retry, 4xx and SQLSTATE `22|23|28|42|P0`/`PGRST*` do not), `computeBackoffMs` (exponential + jitter), `RequestTimeoutError` (added 1.8.0)
- `src/core/supabase-auth.ts` — `createSupabaseAuthAdapter` (Supabase Auth implementation)
- `src/core/supabase.ts` — `createGeoglowsSupabaseClient` factory; the `useIdToken` flag was removed in 0.2.0 (Cognito sessions are no longer forwarded as bearer tokens to PostgREST — Supabase Auth uses its own access token). `autoRefreshToken` and `fetchTimeoutMs` (both added 1.8.0) default to current behaviour; a caller that passes neither still constructs `createClient(url, key)` with two arguments
